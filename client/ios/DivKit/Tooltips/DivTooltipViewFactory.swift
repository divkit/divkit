...
public struct DivTooltipViewFactory {
  ...
  @MainActor
  func makeSubstrateView(div: Div, tooltipId: String) async -> VisibleBoundsTrackingView {
    let view = DivView(divKitComponents: divKitComponents)
    let divData = DivData(
      functions: nil,
      logId: "\(tooltipId)_substrate",
      states: [.init(div: div, stateId: 0)],
      timers: nil,
      transitionAnimationSelector: nil,
      variableTriggers: nil,
    ...
    await view.setSource(
      DivViewSource(
        kind: .divData(divData),
        cardId: cardId,
        additionalId: "\(tooltipId)_substrate"
      )
    )
    return view
  }
  ...
  func makeSubstrateView(div _: Div, tooltipId _: String) async -> ViewType {
    self as AnyObject
  }
  #endif
}
