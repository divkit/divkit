...
public final class DivActionVideoTemplate: TemplateValue, Sendable {
  public typealias Action = DivActionVideo.Action
  ...
  private static func resolveOnlyLinks(context: TemplatesContext, parent: DivActionVideoTemplate?) -> DeserializationResult<DivActionVideo> {
  ...
  public static func resolveValue(context: TemplatesContext, parent: DivActionVideoTemplate?, useOnlyLinks: Bool) -> DeserializationResult<DivActionVideo> {
    if useOnlyLinks {
      return resolveOnlyLinks(context: context, parent: parent)
    }
    var actionValue: DeserializationResult<Expression<DivActionVideo.Action>> = { parent?.action?.value() ?? .noValue }()
  ...
  private func mergedWithParent(templates: [TemplateName: Any]) throws -> DivActionVideoTemplate {
    guard let parent = parent, parent != Self.type else { return self }
    guard let parentTemplate = templates[parent] as? DivActionVideoTemplate else {
      throw DeserializationError.unknownType(type: parent)
    }
    let mergedParent = try parentTemplate.mergedWithParent(templates: templates)

    return DivActionVideoTemplate(
      parent: nil,
  ...
  public func resolveParent(templates: [TemplateName: Any]) throws -> DivActionVideoTemplate {
