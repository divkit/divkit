...
public final class DivStateTemplate: TemplateValue, TemplateDeserializable {
  public final class StateTemplate: TemplateValue, TemplateDeserializable {
    ...
    public convenience init(dictionary: [String: Any], templateToType: TemplateToType) throws {
      do {
        self.init(
          ...
          swipeOutActions: try dictionary.getOptionalArray(
            "swipe_out_actions",
            templateToType: templateToType
          )
        )
    ...
    private static func resolveOnlyLinks(
    ...
    ) -> DeserializationResult<DivState.State> {
      ...
      let divValue = parent?.div?.resolveOptionalValue(
        context: context,
        validator: ResolvedValue.divValidator,
        useOnlyLinks: true
      ) ?? .noValue
      let stateIdValue = parent?.stateId?.resolveValue(context: context) ?? .noValue
      let swipeOutActionsValue = parent?.swipeOutActions?.resolveOptionalValue(
        context: context,
        validator: ResolvedValue.swipeOutActionsValidator,
        useOnlyLinks: true
      ) ?? .noValue
      var errors = mergeErrors(
        ...
        divValue.errorsOrWarnings?
          .map { .right($0.asError(deserializing: "div", level: .warning)) },
        stateIdValue.errorsOrWarnings?
          .map { .right($0.asError(deserializing: "state_id", level: .error)) },
        swipeOutActionsValue.errorsOrWarnings?
          .map { .right($0.asError(deserializing: "swipe_out_actions", level: .warning)) }
      )
    ...
    public static func resolveValue(
      ...
      context.templateData.forEach { key, __dictValue in
        ...
        case "swipe_out_actions":
          swipeOutActionsValue = deserialize(
            __dictValue,
            templates: context.templates,
            templateToType: context.templateToType,
            validator: ResolvedValue.swipeOutActionsValidator,
            type: DivActionTemplate.self
          ).merged(with: swipeOutActionsValue)
        ...
        case parent?.swipeOutActions?.link:
          swipeOutActionsValue = swipeOutActionsValue.merged(with: deserialize(
            __dictValue,
            templates: context.templates,
            templateToType: context.templateToType,
            validator: ResolvedValue.swipeOutActionsValidator,
      ...
      if let parent = parent {
        ...
        animationOutValue = animationOutValue
          .merged(with: parent.animationOut?.resolveOptionalValue(
            context: context,
            validator: ResolvedValue.animationOutValidator,
            useOnlyLinks: true
          ))
        divValue = divValue.merged(with: parent.div?.resolveOptionalValue(
          context: context,
          validator: ResolvedValue.divValidator,
          useOnlyLinks: true
        ))
        swipeOutActionsValue = swipeOutActionsValue
          .merged(with: parent.swipeOutActions?.resolveOptionalValue(
            context: context,
            validator: ResolvedValue.swipeOutActionsValidator,
            useOnlyLinks: true
          ))
      ...
      var errors = mergeErrors(
        ...
        divValue.errorsOrWarnings?
          .map { Either.right($0.asError(deserializing: "div", level: .warning)) },
        stateIdValue.errorsOrWarnings?
          .map { Either.right($0.asError(deserializing: "state_id", level: .error)) },
        swipeOutActionsValue.errorsOrWarnings?
          .map { Either.right($0.asError(deserializing: "swipe_out_actions", level: .warning)) }
      )
      if case .noValue = stateIdValue {
        errors
          .append(.right(FieldError(
  ...
  public convenience init(dictionary: [String: Any], templateToType: TemplateToType) throws {
    do {
      self.init(
        ...
        defaultStateId: try dictionary.getOptionalField("default_state_id"),
      ...
      throw DeserializationError.invalidFieldRepresentation(
        field: "div-state_template." + field,
        representation: representation
      )
    }
  }
  ...
  private static func resolveOnlyLinks(
    context: Context,
    parent: DivStateTemplate?
  ) -> DeserializationResult<DivState> {
    ...
    let defaultStateIdValue = parent?.defaultStateId?.resolveOptionalValue(
      context: context,
      validator: ResolvedValue.defaultStateIdValidator
    ) ?? .noValue
    let divIdValue = parent?.divId?.resolveOptionalValue(
      context: context,
      validator: ResolvedValue.divIdValidator
    ) ?? .noValue
    ...
    var errors = mergeErrors(
      ...
      defaultStateIdValue.errorsOrWarnings?
        .map { .right($0.asError(deserializing: "default_state_id", level: .warning)) },
      divIdValue.errorsOrWarnings?
        .map { .right($0.asError(deserializing: "div_id", level: .warning)) },
  ...
  public static func resolveValue(
    ...
    var columnSpanValue: DeserializationResult<Expression<Int>> = parent?.columnSpan?
      .value() ?? .noValue
    var defaultStateIdValue: DeserializationResult<Expression<String>> = parent?.defaultStateId?
      .value() ?? .noValue
    var divIdValue: DeserializationResult<String> = parent?.divId?
      .value(validatedBy: ResolvedValue.divIdValidator) ?? .noValue
    ...
    context.templateData.forEach { key, __dictValue in
      ...
      case "default_state_id":
        defaultStateIdValue = deserialize(
          __dictValue,
          validator: ResolvedValue.defaultStateIdValidator
        ).merged(with: defaultStateIdValue)
      case "div_id":
        divIdValue = deserialize(__dictValue, validator: ResolvedValue.divIdValidator)
          .merged(with: divIdValue)
      case "extensions":
      ...
      case "states":
        statesValue = deserialize(
          ...
          templateToType: context.templateToType,
          validator: ResolvedValue.statesValidator,
          type: DivStateTemplate.StateTemplate.self
      ...
      case parent?.defaultStateId?.link:
        defaultStateIdValue = defaultStateIdValue
          .merged(with: deserialize(__dictValue, validator: ResolvedValue.defaultStateIdValidator))
      case parent?.divId?.link:
        divIdValue = divIdValue
          .merged(with: deserialize(__dictValue, validator: ResolvedValue.divIdValidator))
    ...
    var errors = mergeErrors(
      ...
      defaultStateIdValue.errorsOrWarnings?
        .map { Either.right($0.asError(deserializing: "default_state_id", level: .warning)) },
      divIdValue.errorsOrWarnings?
        .map { Either.right($0.asError(deserializing: "div_id", level: .warning)) },
      extensionsValue.errorsOrWarnings?
  ...
  private func mergedWithParent(templates: Templates) throws -> DivStateTemplate {
    ...
    return DivStateTemplate(
      ...
      transitionAnimationSelector: transitionAnimationSelector ?? mergedParent
