...
public final class DivPatchTemplate: TemplateValue, TemplateDeserializable {
  public final class ChangeTemplate: TemplateValue, TemplateDeserializable {
    ...
    private static func resolveOnlyLinks(
      context: Context,
      parent: ChangeTemplate?
    ) -> DeserializationResult<DivPatch.Change> {
    ...
    private func mergedWithParent(templates _: Templates) throws -> ChangeTemplate {
      self
    }
  ...
  public convenience init(dictionary: [String: Any], templateToType: TemplateToType) throws {
    do {
      ...
      throw DeserializationError.invalidFieldRepresentation(
        field: "div-patch_template." + field,
        representation: representation
      )
    }
  }
  ...
  private static func resolveOnlyLinks(
    context: Context,
    parent: DivPatchTemplate?
  ) -> DeserializationResult<DivPatch> {
    let changesValue = parent?.changes?.resolveValue(
      context: context,
      validator: ResolvedValue.changesValidator,
      useOnlyLinks: true
    ) ?? .noValue
    ...
    var errors = mergeErrors(
      changesValue.errorsOrWarnings?
        .map { .right($0.asError(deserializing: "changes", level: .error)) },
  ...
  public static func resolveValue(
    ...
    var modeValue: DeserializationResult<Expression<DivPatch.Mode>> = parent?.mode?
      .value() ?? .noValue
    context.templateData.forEach { key, __dictValue in
      switch key {
      case "changes":
        changesValue = deserialize(
          __dictValue,
          templates: context.templates,
          templateToType: context.templateToType,
          validator: ResolvedValue.changesValidator,
          type: DivPatchTemplate.ChangeTemplate.self
        ).merged(with: changesValue)
      case "mode":
        modeValue = deserialize(__dictValue, validator: ResolvedValue.modeValidator)
          .merged(with: modeValue)
      case parent?.changes?.link:
        changesValue = changesValue.merged(with: deserialize(
          __dictValue,
          templates: context.templates,
          templateToType: context.templateToType,
          validator: ResolvedValue.changesValidator,
    ...
    if let parent = parent {
      changesValue = changesValue.merged(with: parent.changes?.resolveValue(
        context: context,
        validator: ResolvedValue.changesValidator,
        useOnlyLinks: true
      ))
    ...
    var errors = mergeErrors(
      changesValue.errorsOrWarnings?
        .map { Either.right($0.asError(deserializing: "changes", level: .error)) },
  ...
  private func mergedWithParent(templates _: Templates) throws -> DivPatchTemplate {
    self
  }
