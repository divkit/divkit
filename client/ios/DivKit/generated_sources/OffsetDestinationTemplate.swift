...
public final class OffsetDestinationTemplate: TemplateValue, Sendable {
  ...
  private static func resolveOnlyLinks(context: TemplatesContext, parent: OffsetDestinationTemplate?) -> DeserializationResult<OffsetDestination> {
    ...
    let result = OffsetDestination(
      value: { valueNonNil }()
    )
    return errors.isEmpty ? .success(result) : .partialSuccess(result, warnings: NonEmptyArray(errors)!)
  }

  public static func resolveValue(context: TemplatesContext, parent: OffsetDestinationTemplate?, useOnlyLinks: Bool) -> DeserializationResult<OffsetDestination> {
    if useOnlyLinks {
  ...
  private func mergedWithParent(templates: [TemplateName: Any]) throws -> OffsetDestinationTemplate {
    guard let parent = parent, parent != Self.type else { return self }
    guard let parentTemplate = templates[parent] as? OffsetDestinationTemplate else {
      throw DeserializationError.unknownType(type: parent)
    }
    let mergedParent = try parentTemplate.mergedWithParent(templates: templates)

    return OffsetDestinationTemplate(
      parent: nil,
      value: value ?? mergedParent.value
    )
  }

  public func resolveParent(templates: [TemplateName: Any]) throws -> OffsetDestinationTemplate {
