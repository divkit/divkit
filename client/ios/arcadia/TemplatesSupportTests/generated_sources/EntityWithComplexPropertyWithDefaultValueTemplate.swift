...
public final class EntityWithComplexPropertyWithDefaultValueTemplate: TemplateValue,
  ...
  public final class PropertyTemplate: TemplateValue, TemplateDeserializable {
    ...
    public convenience init(dictionary: [String: Any], templateToType _: TemplateToType) throws {
      do {
        ...
        throw DeserializationError.invalidFieldRepresentation(
          field: "property_template." + field,
          representation: representation
        )
      }
    }

    init(value: Field<String>? = nil) {
      self.value = value
    }

    private static func resolveOnlyLinks(
      context: Context,
      parent: PropertyTemplate?
    )
      -> DeserializationResult<EntityWithComplexPropertyWithDefaultValue.Property> {
      let valueValue = parent?.value?.resolveValue(context: context) ?? .noValue
      var errors = mergeErrors(
        valueValue.errorsOrWarnings?
          .map { .right($0.asError(deserializing: "value", level: .error)) }
      )
    ...
    private func mergedWithParent(templates _: Templates) throws -> PropertyTemplate {
      self
    }
  ...
  public let property: Field<PropertyTemplate>? // default value: EntityWithComplexPropertyWithDefaultValueTemplate.Property(value: "Default text")
  ...
  private static func resolveOnlyLinks(
    context: Context,
    parent: EntityWithComplexPropertyWithDefaultValueTemplate?
  ) -> DeserializationResult<EntityWithComplexPropertyWithDefaultValue> {
  ...
  public static func resolveValue(
    ...
    var propertyValue: DeserializationResult<EntityWithComplexPropertyWithDefaultValue.Property> =
      .noValue
    context.templateData.forEach { key, __dictValue in
      switch key {
      case "property":
        propertyValue = deserialize(
          ...
          validator: ResolvedValue.propertyValidator,
          type: EntityWithComplexPropertyWithDefaultValueTemplate.PropertyTemplate.self
        ).merged(with: propertyValue)
      case parent?.property?.link:
        propertyValue = propertyValue.merged(with: deserialize(
          __dictValue,
          templates: context.templates,
          templateToType: context.templateToType,
          validator: ResolvedValue.propertyValidator,
    ...
    if let parent = parent {
      propertyValue = propertyValue.merged(with: parent.property?.resolveOptionalValue(
        context: context,
        validator: ResolvedValue.propertyValidator,
        useOnlyLinks: true
      ))
  ...
  private func mergedWithParent(templates: Templates) throws
    ...
    guard let parentTemplate =
      templates[parent] as? EntityWithComplexPropertyWithDefaultValueTemplate else {
