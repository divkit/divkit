...
import static com.yandex.div.json.JsonParser.alwaysValid;
import static com.yandex.div.json.JsonParser.alwaysValidList;
import static com.yandex.div.json.JsonParser.doNotConvert;
import static com.yandex.div.json.JsonTemplateParserKt.suppressMissingValueOrThrow;
...
public class JsonTemplateParser {
    ...
    @NonNull
    public static <R, T> Field<ExpressionsList<T>> readExpressionsListField(
            @NonNull JSONObject json,
            @NonNull String key,
            boolean overridable,
        ...
        ExpressionsList<T> opt = JsonParser.readOptionalExpressionsList(json, key, converter, listValidator, alwaysValid(), logger, env, typeHelper);
        if (opt != null) {
            return new Field.Value<>(overridable, opt);
        } else {
            String reference = readReference(json, key, logger, env);
    ...
    public static <T> Field<ExpressionsList<T>> readExpressionsListField(
            ...
            @NonNull ParsingEnvironment env,
            @NonNull final TypeHelper<T> typeHelper) {
        return readExpressionsListField(json, key, overridable, fallback, doNotConvert(), listValidator, logger, env,
                                        typeHelper);
    }
