...
class HistogramUtilsTest {
    ...
    fun testCalculateUtf8StringByteSize() {
        var str = ""
        assertEquals(str.utf8BytesSize(), calculateUtf8StringByteSize(str))
    ...
    @Test
    fun `calculateUtf8JsonByteSize() of empty JSON`() {
        val json = JSONObject()
        assertEquals(json.toString().utf8BytesSize(), calculateUtf8JsonByteSize(json))
    }

    @Test
    fun `calculateUtf8JsonByteSize() of nested JSON`() {
        val json = JSONObject(
            """
            {
    ...
    @Test
    fun `calculateUtf8JsonByteSize() of real div JSON`() {
        // Android escapes each '/' as '\/', unlike the JVM org.json library used in this test.
        // Use the measured Android size: 10261 bytes + 206 escaped slashes, rather than JVM json.toString().
        assertEquals(10467, calculateUtf8JsonByteSize(JSONObject(REAL_DIV_JSON)))
    }
}
