...
public class ObserverList<E> implements Iterable<E> {
    ...
    /**
     * Returns {@link RewindableIterator} instance that iterate in reversed order.
     */
    ...
    private class ObserverListReversedIterator implements RewindableIterator<E> {
        ...
        private ObserverListReversedIterator() {
            ObserverList.this.incrementIterationDepth();
            mIndex = ObserverList.this.capacity() - 1;
        }
        ...
        public E next() {
            // Advance if the current element is null.
            while (mIndex >= 0 && ObserverList.this.getObserverAt(mIndex) == null) {
                mIndex--;
            }
            if (mIndex >= 0) return ObserverList.this.getObserverAt(mIndex--);
