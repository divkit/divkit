...
/**
 * Class that holds shimmer extension params.
 */
@InternalApi
data class ShimmerExtensionParams(
    ...
    companion object {
        fun fromJson(params: JSONObject?): ShimmerExtensionParams {
            val json = params ?: return ShimmerExtensionParams()
            val logger = ParsingErrorLogger.LOG
            val env = DivParsingEnvironment(logger)
            return ShimmerExtensionParams(
...
private val positiveValidator = ValueValidator<Double> { it > 0 }
