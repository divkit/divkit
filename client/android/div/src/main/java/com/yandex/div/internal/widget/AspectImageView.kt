...
open class AspectImageView @JvmOverloads constructor(
    ...
    var imageScale: Scale = Scale.NO_SCALE
        set(value) {
            if (field != value) {
                field = value
                invalidate()
    ...
    private fun updateMatrix() {
        ...
        val availableWidth = availableWidth.toFloat()
        val availableHeight = availableHeight.toFloat()
        val imageWidth = drawable.intrinsicWidth.toFloat()
        val imageHeight = drawable.intrinsicHeight.toFloat()
        val absoluteGravity = GravityCompat.getAbsoluteGravity(gravity, getLayoutDirection())

        val scaleX = when (imageScale) {
            Scale.NO_SCALE -> {
                currentBitmapWithoutFilters?.let {
                    it.width * context.resources.displayMetrics.density / imageWidth
                } ?: 1f
            }
