...
/// Modified [LocalFileComparator] for golden tests on CI.
///
/// Differences:
///
///  1. Adds this Wiki page to error output
///     https://wiki.yandex-team.ru/taxi/backend/logistics/pro-mobile/cargo-v-platforme/prorabotka-golden-testov/how-to-golden-test/
///
...
///
class CIFileComparator extends LocalFileComparator {
  CIFileComparator(String testFile)
      : _path = _getPath(null),
    ...
    if (!result.passed) {
      final error = await generateFailureOutput(
        ComparisonResult(
          ...
          diffs: result.diffs,
          error: '${result.error}\n'
              'For useful info, check $_wikiPageLink',
        ),
        golden,
        basedir,
      );
