...
class DivScaleTransition(BaseDiv):
    ...
    pivot_x: typing.Optional[float] = Field(description='Relative coordinate `X` of the point that won\'t change its position in case ofscaling.')
    pivot_y: typing.Optional[float] = Field(description='Relative coordinate `Y` of the point that won\'t change its position in case ofscaling.')
    scale: typing.Optional[float] = Field(description='Value of the scale  from which the element starts appearing or at which itfinishes disappearing.')
