...
from . import div_pager_layout_mode
...
class DivPager(BaseDiv):
    ...
    default_item: typing.Optional[int] = Field(description='Ordinal number of the pager element that will be opened by default.')
    ...
    item_spacing: typing.Optional[div_fixed_size.DivFixedSize] = Field(description='Spacing between elements.')
    items: typing.List[div.Div] = Field(min_items=1, description='Pager elements. Page-by-page transition options can be implementedusing:`div-action://set_current_item?id=&item=` — set the current page with anordinal number `item` inside an element, with the specified`id`;`div-action://set_next_item?id=[&overflow={clamp|ring}]` — go to the nextpage inside an element, with the specified`id`;`div-action://set_previous_item?id=[&overflow={clamp|ring}]` — go to theprevious page inside an element, with the specified `id`.</p><p>The optional`overflow` parameter is used to set navigation when the first or last element isreached:`clamp` — transition will stop at the border element;`ring` — go to thebeginning or end, depending on the current element.</p><p>By default, `clamp`.')
    layout_mode: div_pager_layout_mode.DivPagerLayoutMode = Field(description='Type of calculation of the main page width:`fixed` — from the fixed width of thenext page `neighbour_page_width`;`percentage` — from the percentage value`page_width`.')
    margins: typing.Optional[div_edge_insets.DivEdgeInsets] = Field(description='External margins from the element stroke.')
    orientation: typing.Optional[DivPagerOrientation] = Field(description='Pager orientation.')
    paddings: typing.Optional[div_edge_insets.DivEdgeInsets] = Field(description='Internal margins from the element stroke.')
    restrict_parent_scroll: typing.Optional[bool] = Field(description='If the parameter is enabled, the pager won\'t transmit the scroll gesture to theparent element.')
