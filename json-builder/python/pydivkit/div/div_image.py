...
class DivImage(BaseDiv):
    ...
    appearance_animation: typing.Optional[div_fade_transition.DivFadeTransition] = Field(description='Transparency animation when loading an image.')
    ...
    high_priority_preview_show: typing.Optional[bool] = Field(description='It sets the priority of displaying the preview — the preview is decoded in themain stream and displayed as the first frame. Use the parameter carefully — itwill worsen the preview display time and can worsen the application launch time.')
    ...
    image_url: str = Field(format="uri", description='Direct URL to an image.')
    ...
    placeholder_color: typing.Optional[str] = Field(format="color", description='Placeholder background before the image is loaded.')
    preload_required: typing.Optional[bool] = Field(description='Background image must be loaded before the display.')
    preview: typing.Optional[str] = Field(min_length=1, description='Image preview encoded in `base64`. It will be shown instead of`placeholder_color` before the image is loaded. Format `data url`:`data:[;base64],<data>`')
    row_span: typing.Optional[int] = Field(description='Merges cells in a string of the [grid](div-grid.md) element.')
    scale: typing.Optional[div_image_scale.DivImageScale] = Field(description='Image scaling:`fit` places the entire image into the element (free space isfilled with background);`fill` scales the image to the element size and cuts offthe excess.')
