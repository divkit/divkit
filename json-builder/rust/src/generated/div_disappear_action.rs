...
div_entity! {
    ...
    pub struct DivDisappearAction {
        fields: {
            disappear_duration: i64 => FieldSchemaInfo { schema_type: SchemaFieldType::AnyOf(vec![SchemaFieldType::Integer, SchemaFieldType::Expr]), description: Some("Time in milliseconds during which an element must be outside the visible area to trigger `disappear-action`."), default_value: Some(serde_json::json!(800)), constraints: Constraints { minimum: Some(0.0), ..Constraints::default() } },
            ...
            visibility_percentage: i64 => FieldSchemaInfo { schema_type: SchemaFieldType::AnyOf(vec![SchemaFieldType::Integer, SchemaFieldType::Expr]), description: Some("Percentage of the visible part of an element that triggers `disappear-action`."), default_value: Some(serde_json::json!(0)), constraints: Constraints { minimum: Some(0.0), exclusive_maximum: Some(100.0), ..Constraints::default() } },
        }
    }
}
