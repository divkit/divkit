...
div_entity! {
    ...
    pub struct DivPageTransformationOverlap {
        ...
        fields: {
            ...
            reversed_stacking_order: bool => FieldSchemaInfo { schema_type: SchemaFieldType::AnyOf(vec![SchemaFieldType::Boolean, SchemaFieldType::Expr]), description: Some("If the value is `false`, the following pages will be stacked on top of the previous ones. If the value is `true`, the reverse happens. The following page is always the page with a larger ordinal number in the `items` list, regardless of the scrolling direction."), default_value: Some(serde_json::json!(false)), constraints: Constraints::default() },
        }
    }
}
