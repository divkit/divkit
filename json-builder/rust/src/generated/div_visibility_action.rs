...
div_entity! {
    ...
    pub struct DivVisibilityAction {
        fields: {
            ...
            visibility_duration: i64 => FieldSchemaInfo { schema_type: SchemaFieldType::AnyOf(vec![SchemaFieldType::Integer, SchemaFieldType::Expr]), description: Some("Time in milliseconds during which an element must be visible to trigger `visibility-action`."), default_value: Some(serde_json::json!(800)), constraints: Constraints { minimum: Some(0.0), ..Constraints::default() } },
            visibility_percentage: i64 => FieldSchemaInfo { schema_type: SchemaFieldType::AnyOf(vec![SchemaFieldType::Integer, SchemaFieldType::Expr]), description: Some("Percentage of the visible part of an element that triggers `visibility-action`."), default_value: Some(serde_json::json!(50)), constraints: Constraints { exclusive_minimum: Some(0.0), maximum: Some(100.0), ..Constraints::default() } },
        }
    }
}
