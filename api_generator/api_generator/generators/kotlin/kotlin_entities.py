...
class KotlinEntity(Entity):
    ...
    def declare_additional_fields(self) -> Optional[Text]:
        if not isinstance(self.generator_properties, KotlinGeneratorProperties):
            return None

        additional_fields = self.generator_properties.additional_fields
        if not isinstance(additional_fields, list) or len(additional_fields) == 0:
            return None

        result = EMPTY
        for field in filter(lambda f: isinstance(f, Dict), additional_fields):
            field_name = field.get('name')
            field_type = field.get('type')
            if field_name is None or len(field_name) == 0 or field_type is None or len(field_type) == 0:
                continue

            decl = f'var {field_name}: {field_type}'
            field_override = field.get('protocol_name')
            if field_override and len(field_override) != 0:
                if field_override != self.name:
                    decl = f'override {decl}'
                    decl += self.__add_default_value_to_additional_field(field)
            else:
                decl += self.__add_default_value_to_additional_field(field)

            result += Text(decl).indented(indent_width=4)
        return result

    @staticmethod
    def __add_default_value_to_additional_field(field: Dict[str, Any]) -> str:
        default_value = field.get("default_value")
        return f' = {default_value}' if default_value else ''
