...
export type GetTranslationSuggest = (
  query: string,
  locale: string
) => Promise<TranslationVariant[]>;

export type GetTranslationKey = (
  key: string
...
export const DivProEditor = {
  init(opts: DivProEditorOptions): DivProEditorInstance {
    ...
    if (Array.isArray(json?.card?.variables)) {
      const localPalette = json.card.variables.find(
        (it?: JsonVariable) =>
          it?.type === "dict" && it.name === "local_palette"
      );
      if (
        localPalette &&
        localPalette.value &&
        typeof localPalette.value === "object"
      ) {
